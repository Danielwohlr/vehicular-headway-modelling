...
for z=1002:1050
   ...
   B=[0 1];%vynechanej prvek z {0,1,2} mi zustane v datech
...
vysledek_2=[];% tady ted strcim vsechny ty ulozene soubory (levy soupec Density, pravy sloupec intensity)
            %do jednoho dlouheho souboru, ktery zpracuji
for j=1:41
    for k= 248*(j-1) +1: 248*j %postupne z každé matice v ID ulozim data do dlouhe array s nazvem vysledek
        vysledek_2(k,1) = ID(k - 248*(j-1),1,j);
        vysledek_2(k,2) = ID(k - 248*(j-1),2,j);
    end
end
